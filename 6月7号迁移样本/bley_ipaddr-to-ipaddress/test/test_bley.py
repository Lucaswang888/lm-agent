...
class BleyTestCase(unittest.TestCase):
    ...
    ipv4net = ipaddress.ip_network('192.0.2.0/24')
    ipv4generator = ipv4net.hosts()
    ipv6net = ipaddress.ip_network('2001:DB8::/32')
    ipv6generator = ipv6net.hosts()

    def _get_next_ipv4(self):
        try:
            return next(self.ipv4generator)
        except StopIteration:
            self.ipv4generator = self.ipv4net.hosts()
            return next(self.ipv4generator)

    def _get_next_ipv6(self):
        try:
            return next(self.ipv6generator)
        except StopIteration:
            self.ipv6generator = self.ipv6net.hosts()
            return next(self.ipv6generator)
