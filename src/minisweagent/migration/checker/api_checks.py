"""Ground-truth API checks used by the L1 static checker."""
...
def check_api_changes(project: Path, api_changes: list[ApiChange]) -> list[ApiCheckResult]:
    """Check PyMigBench-style source and target snippets using normalised code text."""
...
def _source_removed_for_change(text: str, change: ApiChange) -> bool:
    ...
    return not any(api and re.search(rf"\b{re.escape(api)}\b", text) for api in change.source_apis)
...
def _target_present_for_change(text: str, change: ApiChange) -> bool:
    ...
    return any(api and _target_api_present(text, api) for api in change.target_apis)


def _target_api_present(text: str, api: str) -> bool:
    if "." in api and re.search(rf"\b{re.escape(api)}\b", text):
        return True
    leaf = api.rsplit(".", 1)[-1]
    return len(leaf) >= 3 and bool(re.search(rf"\b{re.escape(leaf)}\b", text))
